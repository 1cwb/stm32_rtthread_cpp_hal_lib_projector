...
public:
    ...
    virtual mResult write(uint16_t slaveAddr, uint8_t* buff, size_t len){return M_RESULT_EOK;}
    virtual mResult read(uint16_t slaveAddr, uint8_t* buff, size_t len){return M_RESULT_EOK;}
    virtual mResult writeReg(uint16_t slaveAddr, uint8_t reg, uint8_t* buff, size_t len){return M_RESULT_EOK;}
