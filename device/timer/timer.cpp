...
mResult timerx::baseTimeInit(const mDev::initCallbackExt& cb, TIM_HandleTypeDef* TimHandle)
{
    _initcb = cb;
...
uint32_t timerx::getInputClk(TIM_TypeDef* TIMx)
{
    uint32_t uiTIMxCLK;
    RCC_ClkInitTypeDef  RCC_ClkInitStruct;
    uint32_t pFLatency;
...
{
    if(htim == timerx::GetObjectFromPrivateMember(htim)->getTimHandle()) {
        timerx::GetObjectFromPrivateMember(htim)->runInitCallback(true);
    }
}
extern "C" void HAL_TIM_Base_MspDeInit(TIM_HandleTypeDef *htim)
{
    if(htim == timerx::GetObjectFromPrivateMember(htim)->getTimHandle()) {
        timerx::GetObjectFromPrivateMember(htim)->runInitCallback(false);
    }
}