...
void QMC5883LCompass::init(){
	_writeReg(QMC5883L_RESET,0x01);
	setMode(MODE_CONTINUES,ODR_200Hz,SCALE_RANGE_8G,OSR_512);
}
...
void QMC5883LCompass::setMode(uint8_t mode, uint8_t odr, uint8_t rng, uint8_t osr){
	_writeReg(QMC5883L_CONFIG,mode|odr|rng|osr);
}
...
void QMC5883LCompass::setReset(){
	_writeReg(QMC5883L_CONFIG2,0x80);
}
...
void QMC5883LCompass::calibrate() {
	...
	unsigned long startTime = mClock::getInstance()->tickGet();

	while((mClock::getInstance()->tickGet() - startTime) < 10000) {
		read();
...
/**
	...
	@return int azimuth
**/
int QMC5883LCompass::getAzimuthData(){
	float heading = atan2( getY(), getX() ) * 180.0 / 3.1415926f;
